...
sino_DC = DotCounting(ph_DS, dot_interval, ray_interval, N, theta, let);
sino_DC_Anal = DotCountingAnal(A, rx, ry, x0, y0, alpha, dot_interval * 20/N, ray_interval * 20/N, N, theta, let);
...
title('Vertical Profile');
subplot(3, 3, 7);
imshow(FBP_DC_Anal, [-threshold threshold]);
title('Reconstruction Image DotCounting Analytic');
subplot(3, 3, 8);
plot(FBP_DC_Anal(N/2 - 10 * y0, :));
grid on;
title('Horizontal Profile');
subplot(3, 3, 9);
plot(FBP_DC_Anal(:, N/2 + 10 * x0));
grid on;
title('Vertical Profile');